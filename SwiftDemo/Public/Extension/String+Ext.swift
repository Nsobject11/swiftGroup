...
extension String {
    ...
    public var unsignedValueStr: String {
    ...
    /// - Returns: 日期字符串
    public static func k_timeStampToDateString(_ timeStamp: String, output: String = " HH:mm") -> String {
    ...
    public func k_toCGFloat() -> CGFloat {
        ...
        let sting : NSMutableString = NSMutableString.init(string: self).mutableCopy() as! NSMutableString
        
        var str : NSString = NSString.init()
        if sting.contains("%") {
            str = sting.replacingOccurrences(of: "%", with: "") as NSString
        }
        if sting.contains("+") {
            str = sting.replacingOccurrences(of: "+", with: "") as NSString
        }
        ...
        return CGFloat(sting.doubleValue)
    }
