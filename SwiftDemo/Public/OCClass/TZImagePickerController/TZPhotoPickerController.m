...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        ...
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) { [photos addObject:@1];
            TZAssetModel *model = tzImagePickerVc.selectedModels[i];
            if ([model.asset isKindOfClass:[NSDictionary class]]) {
                NSDictionary * dic = (NSDictionary *)model.asset;
        ...
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ...
            if ([model.asset isKindOfClass:[PHAsset class]]) {
                ...
                [photos replaceObjectAtIndex:i withObject:assets[i][@"name"]];
                if (i==tzImagePickerVc.selectedModels.count-1) {
                    [tzImagePickerVc hideAlertView:alertView];
                    [self didGetAllPhotos:photos assets:assets infoArr:infoArr];
                }
            }
